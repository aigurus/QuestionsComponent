...
<?php endif;?>
					...
					<div style="background-color:#FFFFFF; border: 1px solid #D4D4D4; padding:30px; border-radius: 10px;">
						<form enctype="multipart/form-data" action="index.php" method="post" name="adminForm" id="themes-form" class="form-validate">
							...
							<div class="control-group">
								<label for="install_package"><b><?php echo JText::_( 'COM_QUESTIONS_UPLOAD_THEME_PACKAGE_FILE' ); ?></b></label>
								<div class="controls">
	...
	<?php if(version_compare(JVERSION, '3.0', 'lt')) : ?>
										<?php echo JText::_( 'COM_QUESTIONS_UPLOAD_AND_INSTALL' ); ?>
	<?php else : ?>
										<i class="icon-upload icon-white"></i> <?php echo JText::_( 'COM_QUESTIONS_UPLOAD_AND_INSTALL' ); ?>
	<?php endif; ?>
									</button>
								</div>
							</div>
				...
				<div class="span5">
					...
					<div>
						<h4>
							<?php echo JText::_('COM_QUESTIONS_THEME_MANAGER') ?> v1
						</h4>
					</div>
					<br/>
				</div>
		...
		<div class="row-fluid">
			<h2><?php echo JText::_('COM_QUESTIONS_THEMES_LIST_TITLE'); ?></h2>
				...
				if($dossier = opendir($url)) {
					while(false !== ($pack = readdir($dossier))) {
						if($pack != '.' && $pack != '..' && $pack != 'index.php' && $pack != 'index.html' && $pack != '.DS_Store' && $pack!='.thumbs') {
							...
							$unknown = JText::_('COM_QUESTIONS_THEME_UNKNOWN');
...
//							foreach ($getUpdatestatus AS $status)
							...
							echo '<div class="span3" style="padding: 10px; margin:10px 20px 10px 0px; background: #D9D9D9; border-radius:10px;">';
								...
								if (file_exists($thumb))
								{
									...
									{
										$imgtheme	= '<div style="text-align:center; max-width=280px"><a href="'.$preview.'" class="modal" title="'.JText::_('COM_QUESTIONS_CLICK_TO_ENLARGE').'">'.$img.'</a></div>';
									}
								} else {
									$imgtheme ='<div style="text-align:center; max-width=280px">'.JText::_('COM_QUESTIONS_THEME_NO_PREVIEW').'</div>';
								}
								...
								// Affichage Auteur
								echo '<div>'.JText::_('COM_QUESTIONS_THEME_AUTHOR').' : <a href="mailto:'.$authorEmail->firstChild->nodeValue.'">' . $author->firstChild->nodeValue . '</a></div>';

								// Affichage Site Auteur
								$authorWebsite = $authorWebsite->firstChild->nodeValue;
								if ($authorWebsite != NULL) {
									echo '<div>'.JText::_('COM_QUESTIONS_THEME_AUTHOR_WEBSITE').' : <a href="'.$authorUrl->firstChild->nodeValue.'" target="_blanck">' . $authorWebsite . '</a></div>';
								}

								// Affichage Version installée
								echo '<div>'.JText::_('COM_QUESTIONS_THEME_INSTALLED_VERSION').' : ' . $version->firstChild->nodeValue . '</div>';

								// Affichage Dernière version publiée
								if (($updateVersion > $version->firstChild->nodeValue) && ($updateVersion != $unknown)) {
									echo '<div>'.JText::_('COM_QUESTIONS_THEME_LATEST_VERSION').' : ' . $updateVersion . '</div></p>';
								}

								echo '<p></p><div style="display:block; margin-left:auto; margin-right: auto;">';

									if (($updateVersion > $version->firstChild->nodeValue) && ($updateVersion != $unknown)) {
										echo '<a href="'.$updateDownload.'" target="_blanck"><div class="btn_update">'.JText::_('COM_QUESTIONS_THEME_UPDATE').' ' . $updateVersion . ' !</div></a>';
									} elseif ($updateVersion == $unknown) {
										echo '<div style="text-align:center; background:#333333; padding:5px; border-radius:5px; color:#FFFFFF;">'.JText::_('COM_QUESTIONS_THEME_AUTHOR_CONTACT').'</div>';
									} else {
										echo '<div style="text-align:center; background:#FFFFFF; padding:5px; border-radius:5px;">'.JText::_('COM_QUESTIONS_THEME_LATEST').'</div>';
									}
						...
						echo '<div>' . JText::_('COM_QUESTIONS_THEME_NB_THEMES_1') . '<strong> ' . $nb_themes . ' </strong>' . JText::_('COM_QUESTIONS_THEME_NB_THEMES_2') .'</div>';
						echo '<div>&nbsp;</div>';
			...
			<div class="span12" style="margin-left: 0px">
				<div class="span6">
					<div>
						<a href="http://www.joomlic.com/translations" target="_blanck" class="btn"><?php echo JText::_('COM_QUESTIONS_PANEL_TRANSLATION_PACKS_DONWLOAD');?></a>
						<a href='http://www.joomlic.com/forum/icagenda'  target="_blanck" class="btn"><?php echo JText::_('COM_QUESTIONS_PANEL_HELP_FORUM'); ?></a>
					</div>
				</div>
				<div class="span6">
				</div>
	...
	<div class="row-fluid">
		...
		<hr>
			<div class="span9">
				...
				<?php echo JText::_('COM_QUESTIONS_PANEL_COPYRIGHT');?>&nbsp;<a href="http://extensions.joomla.org/extensions/calendars-a-events/events/events-management/22013" target="_blanck">Joomla! Extensions Directory</a>.
				<br />
				<br />
			</div>
			<div class="span3" style="text-align: right">
				...
				<br />
				<i><b><?php echo JText::_('COM_QUESTIONS_PANEL_SITE_VISIT');?>&nbsp;<a href='http://www.joomlic.com' target='_blanck'>www.joomlic.com</a></b></i>
			</div>
		</div>
	</div>
