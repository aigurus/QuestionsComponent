...
jQuery(document).on('click', ".removebtnuser", function($) {
...
jQuery(document).on('click', ".removebtngrp", function() {
	var liId = jQuery(this).attr("gid");
	jQuery("span[id=group"+liId+"]").remove();
	jQuery("input[gid="+liId+"]").remove();
	var foos = [];
	document.getElementById("groupids").innerHTML="";
	jQuery("#logs").find("span").each(function(){ foos.push(this.id); });
	document.getElementById("groupids").innerHTML=foos;
	alert("group"+liId+" Deleted");
});
